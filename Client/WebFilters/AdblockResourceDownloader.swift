...
class AdblockResourceDownloader {
    ...
    func regionalAdblockResourcesSetup() async throws {
        ...
        try await downloadResources(type: .regional(locale: locale),
    ...
    func generalAdblockResourcesSetup() async throws {
        try await downloadResources(type: .general,
    ...
    private func downloadResources(type: AdblockerType, queueName: String) async throws {
        ...
        async let completedDownloads = type.associatedFiles.asyncCompactMap { fileType -> AdBlockNetworkResource? in
            ...
            return AdBlockNetworkResource(resource: resource,
                                          fileType: fileType,
                                          type: type)
        }
    ...
    private func setUpFiles(resources: [AdBlockNetworkResource], compileJsonRules: Bool) async throws {
        try await resources.asyncConcurrentForEach {
            switch $0.fileType {
            case .dat:
                try await AdBlockStats.shared.setDataFile(data: $0.resource.data,
